...
const UpdatePropertyPage = () => {
  const { id } = useParams();
  const { getProperty, isLoading: isGettingProperty, error: propertyError, property } = usePropertyStore();
  const { updateProperty, isLoading, error } = useAdminOpsStore();
  ...
  useEffect(() => {
    if (property) {
      ...
      if (property.images && Array.isArray(property.images)) {
        ...
        setImages(property.images); // Assuming images are URLs, if they are objects we might need processing
      }
    }
  }, [property]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Maybe redirect or show success message? updateProperty already shows toast.
    } catch (error) {
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              ...
              <p className="text-gray-600">Fill in the details to update the property listing.</p>
            </div>
          </div>
          ...
          {propertyError && <ErrorLogger error={propertyError} />}
